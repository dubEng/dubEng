...
public class VideoRepositoryImpl implements VideoRepositoryCustom{
    ...
    public Page<CommunityDetailScriptRes> findByAllCommunity(String langType, Pageable pageable, Long recordId){
        ...
        List<CommunityDetailRes> content = queryFactory
                .select(new QCommunityDetailRes(video.id, video.title, video.thumbnail, video.videoPath, video.createdDate, QRecord.record.likeCount, recordComment.id.count(), user.id, user.nickname, QRecord.record.id, video.startTime, video.endTime, QRecord.record.recordPath))
                .from(video)
        ...
        for(CommunityDetailRes c : content){
            CommunityDetailScriptRes cd = new CommunityDetailScriptRes(c.getId(), c.getTitle(), c.getThumbnail(), c.getVideoPath(), c.getCreatedDate(), c.getRecordCommentCount(),c.getUserId(), c.getNickname(), c.getRecordId(), selectAllScript(c.getId()), c.getStartTime(), c.getEndTime(), c.getRecordPath());
