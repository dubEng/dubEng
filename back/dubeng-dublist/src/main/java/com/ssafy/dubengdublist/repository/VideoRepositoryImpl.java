...
public class VideoRepositoryImpl implements VideoRepositoryCustom{
    ...
    public Page<CommunitySearchRes> findByCategoryCommunity(String langType, String  title, Pageable pageable, List<Long> contentsSearch){
        ...
        List<CommunitySearchRes> content = queryFactory
                .select(new QCommunitySearchRes(video.id, video.title,video.thumbnail,video.runtime, user.nickname, user.profileImage, QRecord.record.playCount, QRecord.record.updatedDate, QRecord.record.id))
                .from(video)
                ...
                .orderBy(QRecord.record.updatedDate.desc())
                .offset(pageable.getOffset())
                .limit(pageable.getPageSize())
    ...
    public CommunityDubKingRes findByOneDubKing(String langType, String userId){
        ...
        ContentsDetailRes content = queryFactory
                ...
                .where(video.langType.eq(langType), QRecord.record.isPublic.eq(true), QRecord.record.user.id.ne(userId), QRecord.record.user.isActive.eq(true))
                .groupBy(video.id)
